(() => {
  ...
  function activityTimestamp(item){
    const value=item?.createdAt||item?.timestamp||item?.at;
    const time=value?Date.parse(value):NaN;
    return Number.isFinite(time)?time:0;
  }
  function normalizeFamilyHistory(items){
    const seen=new Set();
    return (Array.isArray(items)?items:[]).map((item,index)=>{
      if(typeof item==='string')item={icon:'•',text:item,time:'Раніше'};
      const text=String(item?.text||'').trim();if(!text)return null;
      const createdAt=item.createdAt||item.timestamp||item.at||new Date(Date.now()-index*1000).toISOString();
      const id=item.id||`activity-${createdAt}-${text}`;
      return {...item,id,createdAt,time:item.time||new Date(createdAt).toLocaleString('uk-UA')};
    }).filter(Boolean).filter(item=>{if(seen.has(item.id))return false;seen.add(item.id);return true;})
      .sort((a,b)=>activityTimestamp(b)-activityTimestamp(a)).slice(0,100);
  }
  function mergeFamilyHistory(localItems,remoteItems){
    return normalizeFamilyHistory([...(localItems||[]),...(remoteItems||[])]);
  }
  function addFamilyActivity(icon,text,actor=currentUser()){
    if(actor&&(actor.role==='admin'||actor.role==='owner'))return;
    const createdAt=new Date().toISOString();
    state.history=normalizeFamilyHistory([{id:`activity-${createdAt}-${Math.random().toString(36).slice(2,8)}`,icon:icon||'•',text,createdAt,time:'Щойно'},...(state.history||[])]);
  }
  ...
  function normalizeState(){
    ...
    state.history=normalizeFamilyHistory(state.history);
    // Прибираємо автоматично згенеровані повторювані «етапи» та семантичні дублікати.
    const seenAchievements=new Set();
    state.achievements=(Array.isArray(state.achievements)?state.achievements:[]).filter(a=>{
      if(!a||/^achievement_[a-z]+_\d+$/i.test(String(a.id||''))||/—\s*етап\s*\d+/i.test(String(a.title||'')))return false;
      const key=(String(a.title||'')+'|'+String(a.description||'')).toLocaleLowerCase('uk-UA').replace(/[^a-zа-яіїєґ0-9]+/gi,' ').trim();
      if(!key||seenAchievements.has(key))return false;seenAchievements.add(key);return true;
    });
    ...
    for(const [index,u] of (state.users||[]).entries()){
      ...
      if(telegramInitData&&u.telegramLinked){for(const id of ['tg_first_login','tg_verified_profile'])if(!u.achievements.includes(id))u.achievements.push(id);}
      if(!telegramInitData)u.achievements=u.achievements.filter(id=>!String(id).startsWith('tg_'));
    }
    const extra=[
    ...
    for(const a of extra)if((telegramInitData||!String(a.id).startsWith('tg_'))&&!state.achievements.some(x=>x.id===a.id))state.achievements.push(a);
  ...
  function addXp(u,amount,source='Активність'){
    ...
    while(u.xp>=xpRequiredForLevel(u.level)){
      ...
      addFamilyActivity('⭐',`${u.name} досяг(ла) ${u.level} рівня`,u);
    }
  ...
  function mergeContent(content){
    ...
    state.quests=merge(state.quests||[],content.quests||[]);const curatedAchievements=(content.achievements||[]).filter(a=>!/^achievement_[a-z]+_\d+$/i.test(String(a.id||''))&&!/—\s*етап\s*\d+/i.test(String(a.title||'')));state.achievements=merge(state.achievements||[],curatedAchievements);state.shop=merge(state.shop||[],content.shop||[]);
  ...
  async function refreshPublicMeta(){
    try{const data=await fetch('/api/app-meta').then(r=>r.ok?r.json():Promise.reject());if(data?.updateName){publicUpdateName=data.updateName;document.querySelectorAll('.release-label').forEach(x=>x.textContent=publicUpdateName);}}catch{}
  }
  ...
  async function pullRemote(){
    ...
    try{const localHistory=state.history||[];const data=await api('/api/family/state');if(data.state){state=data.state;state.history=mergeFamilyHistory(localHistory,data.state.history||[]);normalizeState();localStorage.setItem(STORAGE,JSON.stringify(state));localStorage.setItem(LAST_SERVER_PULL,new Date().toISOString());}}catch{}
  }
  ...
  function unlockAchievement(u,id){if(!u||u.achievements.includes(id))return false;u.achievements.push(id);const a=state.achievements.find(x=>x.id===id);u.activity.unshift(`Отримано ачивку «${a?.title||id}»`);addFamilyActivity(a?.icon||'🏆',`${u.name} отримав(ла) «${a?.title||'нову ачивку'}»`,u);if(u.id===state.currentUserId)queueAchievementToast(a||{id,icon:'🏆',title:id,rarity:'Звичайна'});return true;}
  ...
  function sendMuseumGift(){
    ...
    state.giftHistory.push(gift);to.receivedGifts.push(gift);to.activity.unshift(`Отримано подарунок «${title}» від ${from.name}`);from.activity.unshift(`Подаровано «${title}» для ${to.name}`);addFamilyActivity(icon,`${from.name} подарував(ла) ${to.name} «${title}»`,from);addXp(from,10,'подарунок');addXp(to,5,'отриманий подарунок');save();render();showToast(`Подарунок для ${to.name} надіслано`);
  }
  ...
  function handleShop(id){
    ...
    if(item.type==='collective'){
      const contribution=Math.min(u.coins,Math.max(100,Math.ceil((item.price-item.fund)/4)));if(!contribution)return showToast('Недостатньо монет');u.coins-=contribution;item.fund+=contribution;if(item.fund>=item.price){item.stock-=1;item.fund=item.price;addFamilyActivity(item.icon,`Сімʼя зібрала на «${item.title}»`,u);showToast('Спільну ціль досягнуто!');}else showToast(`Внесено ${contribution} монет`);
    }else{if(u.coins<item.price)return showToast('Недостатньо монет');u.coins-=item.price;item.stock-=1;addFamilyActivity(item.icon,`${u.name} придбав(ла) «${item.title}»`,u);showToast('Можливість придбано');}
    save();render();
  }
