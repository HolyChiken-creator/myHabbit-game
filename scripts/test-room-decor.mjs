...
assert.match(appSource,/function roomStatusBar\(u\)/);
assert.match(appSource,/room-status-bar/);
assert.doesNotMatch(appSource,/data-room-studio-drag/);
assert.doesNotMatch(appSource,/data-room-studio-resize/);
assert.match(appSource,/function roomStatusBar\(u\)/);
assert.match(appSource,/room-status-bar/);
assert.doesNotMatch(appSource,/data-room-studio-drag/);
assert.doesNotMatch(appSource,/data-room-studio-resize/);
...
assert.match(homeCss,/12\.11\.0 — mobile-first inline Teddy room editor/);
assert.match(homeCss,/room-inline-studio/);
assert.match(homeCss,/room-status-bar/);
assert.match(homeCss,/12\.11\.0 — mobile-first inline Teddy room editor/);
assert.match(homeCss,/room-inline-studio/);
assert.match(homeCss,/room-status-bar/);
assert.match(homeCss,/grid-template-columns:repeat\(2,minmax\(0,1fr\)\)!important/);
...
console.log('PASS: 15-zone Teddy room + clean viewport + inline scrollable studio + admin decorator mode.');
