...
const ROOM_LAYOUT_LEVELS = ['0','1','2','3','4'];
const ROOM_LAYOUT_SLOTS = ['window','armchair','table','bookshelf','fireplace','rug','plant','painting','lamp','clock','teddy'];
const clampRoomNumber = (value, min, max, fallback = min) => {
  const parsed = Number(value);
  return Number.isFinite(parsed) ? Math.min(max, Math.max(min, parsed)) : fallback;
};
const sanitizeOwnerRoomLayout = (payload = {}) => {
  const raw = payload?.layouts || payload?.roomLayoutMaster || payload;
  const safe = {};
  if (!raw || typeof raw !== 'object') return safe;
  for (const level of ROOM_LAYOUT_LEVELS) {
    const source = raw[level];
    if (!source || typeof source !== 'object') continue;
    const levelSafe = {};
    for (const slot of ROOM_LAYOUT_SLOTS) {
      const box = source[slot];
      if (!box || typeof box !== 'object') continue;
      const next = {
        x: Math.round(clampRoomNumber(box.x, 0, 95, 0) * 1000) / 1000,
        y: Math.round(clampRoomNumber(box.y, 0, 95, 0) * 1000) / 1000,
        w: Math.round(clampRoomNumber(box.w, 5, 80, 18) * 1000) / 1000,
        z: Math.trunc(clampRoomNumber(box.z, 1, 40, 10))
      };
      if (slot !== 'teddy' && Number.isFinite(Number(box.h))) next.h = Math.round(clampRoomNumber(box.h, 3, 95, 20) * 1000) / 1000;
      levelSafe[slot] = next;
    }
    if (Object.keys(levelSafe).length) safe[level] = levelSafe;
  }
  return safe;
};
...
export class TelegramStateV2 {
  ...
  async handleRequest(request) {
    ...
    try {
      ...
      if (url.pathname === '/owner-room-layout' && request.method === 'GET') return this.ownerRoomLayout();
      if (url.pathname === '/owner-room-layout' && request.method === 'PUT') return this.ownerRoomLayoutUpdate(await request.json().catch(() => ({})));
  ...
  async familyAction(request,op) {
    ...
    try {
      if(op.type==='admin-transfer'){
        ...
        const seasonalStickerTesting = ['sticker-box','dust-exchange'].includes(op.type)
          ? Boolean(await this.state.storage.get('owner-seasonal-sticker-testing') || false)
          : false;
        result=applyGameAction(auth.family.state,auth.user.id,op,Date.now(),()=>crypto.getRandomValues(new Uint32Array(1))[0]/4294967296,{seasonalStickerTesting});
      }
...
Щоб додати ще один телефон, натисніть «🔗 Підключити PWA» і отримайте новий код.`,
  ...
  async ownerRoomLayout() {
    const record = await this.state.storage.get('owner-room-layout');
    const layouts = record?.layouts && typeof record.layouts === 'object' ? record.layouts : {};
    return json({
      ok: true,
      configured: Object.keys(layouts).length > 0,
      revision: Number(record?.revision || 0),
      layouts,
      updatedAt: String(record?.updatedAt || ''),
      updatedBy: 'owner-console'
    });
  }

  async ownerRoomLayoutUpdate(body = {}) {
    const layouts = sanitizeOwnerRoomLayout(body);
    if (!Object.keys(layouts).length) return json({ error: 'Layout кімнати порожній або некоректний' }, 400);
    const previous = await this.state.storage.get('owner-room-layout');
    const revision = Number(previous?.revision || 0) + 1;
    const updatedAt = new Date().toISOString();
    const record = { layouts, revision, updatedAt, source: 'owner-console' };
    await this.state.storage.put('owner-room-layout', record);
    if (body?.commit) {
      await this.recordOwnerAction('room-layout-publish', true, `Master-layout кімнати опубліковано: r${revision}`, {
        revision,
        levels: Object.keys(layouts).length,
        source: 'owner-console'
      });
    }
    return json({ ok: true, configured: true, ...record });
  }
...
const roomCfg=window.ROOM_MASTER_CONFIG;
const roomClone=value=>JSON.parse(JSON.stringify(value));
const roomClamp=(value,min,max)=>Math.min(max,Math.max(min,Number(value)||0));
const ownerRoomDraftKey='myhabbit:owner-console:room-draft:v1';
let ownerRoomState={level:0,selected:'armchair',layouts:roomCfg?roomClone(roomCfg.defaults):{},publishedLayouts:roomCfg?roomClone(roomCfg.defaults):{},revision:0,configured:false,updatedAt:'',drag:null,dirty:false,draftTimer:null,publishBusy:false,loaded:false};
function roomNormalize(layouts){
  const out=roomClone(roomCfg.defaults||{});if(!layouts||typeof layouts!=='object')return out;
  for(const level of ['0','1','2','3','4']){
    const src=layouts[level];if(!src||typeof src!=='object')continue;
    for(const slot of [...roomCfg.slots,'teddy']){
      const b=src[slot];if(!b||typeof b!=='object')continue;
      const fallback=out[level]?.[slot]||{};
      out[level][slot]={x:roomClamp(b.x,0,95),y:roomClamp(b.y,0,95),w:roomClamp(b.w,5,80),z:Math.trunc(roomClamp(b.z,1,40)),...(slot!=='teddy'&&Number.isFinite(Number(b.h))?{h:roomClamp(b.h,3,95)}:(Number.isFinite(Number(fallback.h))?{h:fallback.h}:{}))};
    }
  }
  return out;
}
function roomExtractLayouts(parsed){
  if(!parsed||typeof parsed!=='object')throw new Error('JSON має містити об’єкт з layout кімнати');
  const candidate=parsed.layouts||parsed.roomLayoutMaster||parsed.roomMaster?.layouts||parsed.layout||parsed;
  if(!candidate||typeof candidate!=='object')throw new Error('Не знайдено layouts');
  let known=0;
  for(const level of ['0','1','2','3','4']){
    const src=candidate[level];if(!src||typeof src!=='object')continue;
    for(const slot of [...roomCfg.slots,'teddy'])if(src[slot]&&typeof src[slot]==='object')known++;
  }
  if(!known)throw new Error('JSON не містить відомих об’єктів кімнати для рівнів 0–4');
  return roomNormalize(candidate);
}
function roomBox(){return ownerRoomState.layouts[String(ownerRoomState.level)]?.[ownerRoomState.selected]||null}
function roomApplyBox(node,box,slot){if(!node||!box)return;node.style.left=roomClamp(box.x,0,95)+'%';node.style.top=roomClamp(box.y,0,95)+'%';node.style.width=roomClamp(box.w,5,80)+'%';node.style.zIndex=String(Math.trunc(roomClamp(box.z,1,40)));if(slot!=='teddy')node.style.height=Number.isFinite(Number(box.h))?roomClamp(box.h,3,95)+'%':'auto'}
function roomSetLiveStatus(kind,text){const dot=$('ownerRoomLiveDot'),label=$('ownerRoomLiveText');if(dot){dot.classList.toggle('saving',kind==='saving');dot.classList.toggle('error',kind==='error');dot.classList.toggle('dirty',kind==='dirty')}if(label)label.textContent=text||'Локальна чернетка'}
function roomPersistDraft(){if(!roomCfg)return;try{localStorage.setItem(ownerRoomDraftKey,JSON.stringify({version:'12.23.1',savedAt:new Date().toISOString(),baseRevision:Number(ownerRoomState.revision||0),layouts:ownerRoomState.layouts}))}catch{}}
function roomMarkDirty(text='Локальні зміни збережено у чернетку'){ownerRoomState.dirty=true;roomSetLiveStatus('dirty',text);clearTimeout(ownerRoomState.draftTimer);ownerRoomState.draftTimer=setTimeout(roomPersistDraft,120)}
function roomFlushDraft(text){clearTimeout(ownerRoomState.draftTimer);roomPersistDraft();if(text)$('ownerRoomResult').textContent=text}
function roomReadDraft(){try{const raw=localStorage.getItem(ownerRoomDraftKey);if(!raw)return null;const parsed=JSON.parse(raw);return roomExtractLayouts(parsed)}catch(error){$('ownerRoomResult').textContent='Локальну чернетку не вдалося відновити: '+error.message;return null}}
function roomClearDraft(){clearTimeout(ownerRoomState.draftTimer);try{localStorage.removeItem(ownerRoomDraftKey)}catch{}}
function renderOwnerRoom(){
  const stage=$('ownerRoomStage');if(!stage)return;
  if(!roomCfg){stage.innerHTML='<div style="display:grid;place-items:center;height:100%;padding:24px;text-align:center;color:#934d4d;font-weight:800">Не завантажено конфігурацію кімнати. Оновіть сторінку після встановлення 12.23.1.</div>';roomSetLiveStatus('error','Конфігурацію кімнати не завантажено');return}
  const level=String(ownerRoomState.level),cfg=roomCfg.levels[level]||roomCfg.levels[0],layout=ownerRoomState.layouts[level]||roomCfg.defaults[level];
  stage.innerHTML='<img class="owner-room-bg" src="'+cfg.background+'" alt="">'+roomCfg.slots.map(slot=>'<img class="owner-room-object'+(slot===ownerRoomState.selected?' is-selected':'')+'" draggable="false" data-owner-room-slot="'+slot+'" src="'+cfg.assets[slot]+'" alt="'+esc(roomCfg.labels[slot]||slot)+'">').join('')+'<img class="owner-room-object'+(ownerRoomState.selected==='teddy'?' is-selected':'')+'" draggable="false" data-owner-room-slot="teddy" src="/assets/generated/pack_00_style_lock/webp/teddy_front_v01.webp" alt="Тедик">';
  stage.querySelectorAll('[data-owner-room-slot]').forEach(node=>{const slot=node.dataset.ownerRoomSlot;roomApplyBox(node,layout?.[slot],slot);node.onpointerdown=event=>startOwnerRoomDrag(event,node,slot)});
  $('ownerRoomLevels').innerHTML=[0,1,2,3,4].map(n=>'<button class="'+(n===ownerRoomState.level?'active':'')+'" data-owner-room-level="'+n+'">'+n+' · '+esc(roomCfg.levels[n].title)+'</button>').join('');
  $('ownerRoomLevels').querySelectorAll('[data-owner-room-level]').forEach(button=>button.onclick=()=>{ownerRoomState.level=Number(button.dataset.ownerRoomLevel);renderOwnerRoom()});
  const object=$('ownerRoomObject');if(object){object.innerHTML=[...roomCfg.slots,'teddy'].map(slot=>'<option value="'+slot+'">'+esc(roomCfg.labels[slot]||slot)+'</option>').join('');object.value=ownerRoomState.selected;object.onchange=()=>{ownerRoomState.selected=object.value;renderOwnerRoom()}}
  const box=roomBox();if(box){$('ownerRoomSize').value=box.w;$('ownerRoomSizeValue').textContent=(Math.round(box.w*10)/10)+'%';$('ownerRoomZ').value=box.z;$('ownerRoomZValue').textContent=String(box.z)}
  $('roomRevision').textContent='r'+Number(ownerRoomState.revision||0);$('roomUpdated').textContent=ownerRoomState.updatedAt?new Date(ownerRoomState.updatedAt).toLocaleString('uk-UA'):'Ще не опубліковано';
}
function startOwnerRoomDrag(event,node,slot){event.preventDefault();ownerRoomState.selected=slot;const stage=$('ownerRoomStage'),rect=stage.getBoundingClientRect(),box=ownerRoomState.layouts[String(ownerRoomState.level)][slot];ownerRoomState.drag={node,slot,rect,startX:event.clientX,startY:event.clientY,x:box.x,y:box.y,pointerId:event.pointerId};node.setPointerCapture?.(event.pointerId);node.onpointermove=moveOwnerRoomDrag;node.onpointerup=endOwnerRoomDrag;node.onpointercancel=endOwnerRoomDrag;renderOwnerRoomSelectionOnly()}
function renderOwnerRoomSelectionOnly(){const stage=$('ownerRoomStage');stage?.querySelectorAll('[data-owner-room-slot]').forEach(node=>node.classList.toggle('is-selected',node.dataset.ownerRoomSlot===ownerRoomState.selected));const obj=$('ownerRoomObject');if(obj)obj.value=ownerRoomState.selected;const box=roomBox();if(box){$('ownerRoomSize').value=box.w;$('ownerRoomSizeValue').textContent=(Math.round(box.w*10)/10)+'%';$('ownerRoomZ').value=box.z;$('ownerRoomZValue').textContent=String(box.z)}}
function moveOwnerRoomDrag(event){const drag=ownerRoomState.drag;if(!drag)return;const box=ownerRoomState.layouts[String(ownerRoomState.level)][drag.slot];box.x=roomClamp(drag.x+(event.clientX-drag.startX)/drag.rect.width*100,0,92);box.y=roomClamp(drag.y+(event.clientY-drag.startY)/drag.rect.height*100,0,90);roomApplyBox(drag.node,box,drag.slot);ownerRoomState.dirty=true;roomSetLiveStatus('dirty','Редагуєте локальну чернетку')}
function endOwnerRoomDrag(){const drag=ownerRoomState.drag;if(!drag)return;drag.node.onpointermove=null;drag.node.onpointerup=null;drag.node.onpointercancel=null;ownerRoomState.drag=null;roomMarkDirty('Локальна чернетка змінена · ще не опубліковано');roomFlushDraft()}
async function publishOwnerRoom(){
  if(!roomCfg||ownerRoomState.publishBusy)return false;clearTimeout(ownerRoomState.draftTimer);ownerRoomState.publishBusy=true;roomSetLiveStatus('saving','Публікую для всіх пристроїв…');const button=$('ownerRoomPublish');if(button)busy(button,true,'Опублікувати для всіх');try{const d=await api('/api/owner/room-layout',{method:'PUT',body:JSON.stringify({layouts:ownerRoomState.layouts,commit:true})});ownerRoomState.revision=Number(d.revision||ownerRoomState.revision);ownerRoomState.updatedAt=d.updatedAt||ownerRoomState.updatedAt;ownerRoomState.configured=true;ownerRoomState.publishedLayouts=roomClone(ownerRoomState.layouts);ownerRoomState.dirty=false;roomClearDraft();renderOwnerRoom();roomSetLiveStatus('ok','Опубліковано · r'+ownerRoomState.revision);$('ownerRoomResult').textContent='Готово: r'+ownerRoomState.revision+' опубліковано для всіх користувачів. Відкриті пристрої підхоплять новий master-layout.';return true}catch(error){roomSetLiveStatus('error','Помилка публікації');$('ownerRoomResult').textContent=error.message;return false}finally{ownerRoomState.publishBusy=false;if(button)busy(button,false,'Опублікувати для всіх')}}
async function loadOwnerRoomLayout(discardDraft=false){
  if(!roomCfg){renderOwnerRoom();return}
  try{
    const d=await api('/api/owner/room-layout');const serverLayouts=roomNormalize(d.configured?d.layouts:roomCfg.defaults);ownerRoomState.publishedLayouts=roomClone(serverLayouts);ownerRoomState.revision=Number(d.revision||0);ownerRoomState.updatedAt=d.updatedAt||'';ownerRoomState.configured=!!d.configured;ownerRoomState.loaded=true;
    if(discardDraft)roomClearDraft();
    const draft=discardDraft?null:roomReadDraft();
    if(draft){ownerRoomState.layouts=draft;ownerRoomState.dirty=true;renderOwnerRoom();roomSetLiveStatus('dirty','Відновлено локальну чернетку · не опубліковано');$('ownerRoomResult').textContent='Відновлено локальну чернетку. Користувачі її не бачать, доки ви не натиснете «Опублікувати для всіх».'}
    else{ownerRoomState.layouts=roomClone(serverLayouts);ownerRoomState.dirty=false;renderOwnerRoom();roomSetLiveStatus('ok',d.configured?'Завантажено опубліковану версію · r'+ownerRoomState.revision:'Базовий layout · ще не опубліковано');if(discardDraft)$('ownerRoomResult').textContent=d.configured?'Чернетку скинуто до опублікованої r'+ownerRoomState.revision:'Чернетку скинуто до базового layout'}
  }catch(error){ownerRoomState.loaded=true;ownerRoomState.layouts=roomClone(roomCfg.defaults);renderOwnerRoom();roomSetLiveStatus('error','Серверний layout недоступний · працює локальна база');$('ownerRoomResult').textContent='Не вдалося завантажити опублікований layout: '+error.message+'. Можна редагувати базовий варіант локально.'}
}
function nudgeOwnerRoom(dx,dy){const box=roomBox();if(!box)return;box.x=roomClamp(box.x+dx,0,92);box.y=roomClamp(box.y+dy,0,90);renderOwnerRoom();roomMarkDirty('Локальна чернетка змінена · ще не опубліковано')}
function resetOwnerRoomObject(){const level=String(ownerRoomState.level),slot=ownerRoomState.selected;ownerRoomState.layouts[level][slot]=roomClone(roomCfg.defaults[level][slot]);renderOwnerRoom();roomMarkDirty('Предмет скинуто локально · ще не опубліковано')}
function resetOwnerRoomLevel(){const level=String(ownerRoomState.level);ownerRoomState.layouts[level]=roomClone(roomCfg.defaults[level]);renderOwnerRoom();roomMarkDirty('Рівень скинуто локально · ще не опубліковано')}
function exportOwnerRoom(){const blob=new Blob([JSON.stringify({version:'12.23.1',exportedAt:new Date().toISOString(),source:'owner-console-local-draft',baseRevision:Number(ownerRoomState.revision||0),layouts:ownerRoomState.layouts},null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='myhabbit-owner-room-draft-'+new Date().toISOString().slice(0,10)+'.json';a.click();setTimeout(()=>URL.revokeObjectURL(a.href),500);$('ownerRoomResult').textContent='Локальну чернетку експортовано у JSON'}
async function importOwnerRoomFile(file){try{if(!file)throw new Error('Файл не вибрано');if(file.size>2*1024*1024)throw new Error('JSON завеликий — максимум 2 МБ');const text=await file.text();if(!text.trim())throw new Error('Файл порожній');const parsed=JSON.parse(text);ownerRoomState.layouts=roomExtractLayouts(parsed);ownerRoomState.dirty=true;renderOwnerRoom();roomMarkDirty('JSON імпортовано у локальну чернетку');roomFlushDraft('JSON імпортовано локально. Перевірте кімнату і натисніть «Опублікувати для всіх», коли все готово.')}catch(error){roomSetLiveStatus('error','Помилка імпорту JSON');$('ownerRoomResult').textContent='Не вдалося імпортувати JSON: '+error.message}}
function ownerRoomPresetKey(n){return 'myhabbit:owner-console:room-preset:'+n}
function renderOwnerRoomPresets(){$('ownerRoomPresets').innerHTML=[1,2,3].map(n=>'<button class="ghost" data-owner-room-preset-save="'+n+'">Зберегти варіант '+n+'</button><button class="ghost" data-owner-room-preset-load="'+n+'">Підвантажити '+n+'</button>').join('');$('ownerRoomPresets').querySelectorAll('[data-owner-room-preset-save]').forEach(b=>b.onclick=()=>{localStorage.setItem(ownerRoomPresetKey(b.dataset.ownerRoomPresetSave),JSON.stringify({version:'12.23.1',savedAt:new Date().toISOString(),layouts:ownerRoomState.layouts}));$('ownerRoomResult').textContent='Варіант '+b.dataset.ownerRoomPresetSave+' збережено локально'});$('ownerRoomPresets').querySelectorAll('[data-owner-room-preset-load]').forEach(b=>b.onclick=()=>{const raw=localStorage.getItem(ownerRoomPresetKey(b.dataset.ownerRoomPresetLoad));if(!raw){$('ownerRoomResult').textContent='Варіант '+b.dataset.ownerRoomPresetLoad+' порожній';return}try{ownerRoomState.layouts=roomExtractLayouts(JSON.parse(raw));renderOwnerRoom();roomMarkDirty('Локальний варіант '+b.dataset.ownerRoomPresetLoad+' підвантажено · ще не опубліковано');roomFlushDraft('Варіант '+b.dataset.ownerRoomPresetLoad+' підвантажено у чернетку')}catch(error){$('ownerRoomResult').textContent='Не вдалося відкрити варіант: '+error.message}})}
function bindOwnerRoomControls(){if(!roomCfg||!$('ownerRoomStage')){renderOwnerRoom();return}renderOwnerRoomPresets();$('ownerRoomSize').oninput=e=>{const box=roomBox();if(!box)return;const next=roomClamp(e.target.value,5,80);if(ownerRoomState.selected!=='teddy'&&Number.isFinite(Number(box.h)))box.h=roomClamp(box.h*next/box.w,3,95);box.w=next;$('ownerRoomSizeValue').textContent=(Math.round(next*10)/10)+'%';const node=$('ownerRoomStage').querySelector('[data-owner-room-slot="'+ownerRoomState.selected+'"]');roomApplyBox(node,box,ownerRoomState.selected);roomMarkDirty('Редагуєте локальну чернетку')};$('ownerRoomSize').onchange=()=>roomFlushDraft();$('ownerRoomZ').oninput=e=>{const box=roomBox();if(!box)return;box.z=Math.trunc(roomClamp(e.target.value,1,40));$('ownerRoomZValue').textContent=String(box.z);const node=$('ownerRoomStage').querySelector('[data-owner-room-slot="'+ownerRoomState.selected+'"]');roomApplyBox(node,box,ownerRoomState.selected);roomMarkDirty('Редагуєте локальну чернетку')};$('ownerRoomZ').onchange=()=>roomFlushDraft();$('ownerRoomUp').onclick=()=>nudgeOwnerRoom(0,-.5);$('ownerRoomDown').onclick=()=>nudgeOwnerRoom(0,.5);$('ownerRoomLeft').onclick=()=>nudgeOwnerRoom(-.5,0);$('ownerRoomRight').onclick=()=>nudgeOwnerRoom(.5,0);$('ownerRoomResetObject').onclick=resetOwnerRoomObject;$('ownerRoomResetLevel').onclick=resetOwnerRoomLevel;$('ownerRoomPublish').onclick=publishOwnerRoom;$('ownerRoomReload').onclick=()=>{if(!ownerRoomState.dirty||confirm('Відкинути локальну чернетку та завантажити останню опубліковану версію?'))loadOwnerRoomLayout(true)};$('ownerRoomExport').onclick=exportOwnerRoom;$('ownerRoomImport').onclick=()=>$('ownerRoomImportFile').click();$('ownerRoomImportFile').onchange=e=>{const file=e.target.files?.[0];if(file)importOwnerRoomFile(file);e.target.value=''}}
...
function initOwnerFolds(){
  const configs=[
    ...
    ['room-layout-section','Головний вигляд кімнати','Локальна чернетка → публікація на всі пристрої'],
...
$('logout').onclick=async()=>{await api('/api/owner/logout',{method:'POST'});location.reload()};bindOwnerRoomControls();load();setInterval(load,30000);
</script></body></html>`, { headers: { 'content-type':'text/html; charset=utf-8','cache-control':'no-store','x-robots-tag':'noindex, nofollow','content-security-policy':"default-src 'self'; style-src 'unsafe-inline'; script-src 'self' 'unsafe-inline'; img-src 'self' data: blob:; frame-ancestors 'none'" } });
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/api/owner/room-layout' && request.method === 'GET') {
      if (!await ownerAuthorized(request, env)) return json({ error: 'Unauthorized' }, 401);
      return stateStub(env).fetch('https://telegram-state/owner-room-layout');
    }
    if (url.pathname === '/api/owner/room-layout' && request.method === 'PUT') {
      if (!await ownerAuthorized(request, env)) return json({ error: 'Unauthorized' }, 401);
      return stateStub(env).fetch('https://telegram-state/owner-room-layout', { method: 'PUT', headers: { 'content-type': 'application/json' }, body: await request.text() });
    }
    if (url.pathname === '/api/app-room-layout' && request.method === 'GET') {
      return stateStub(env).fetch('https://telegram-state/owner-room-layout');
    }
