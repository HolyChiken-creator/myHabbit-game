...
let ownerRoomState={level:0,selected:'armchair',layouts:roomCfg?roomClone(roomCfg.defaults):{},revision:0,configured:false,updatedAt:'',drag:null,saveTimer:null,lastSent:'',lastLiveAt:0,publishBusy:false,publishQueued:false,publishQueuedCommit:false};
...
function roomSetLiveStatus(kind,text){const dot=$('ownerRoomLiveDot'),label=$('ownerRoomLiveText');if(dot){dot.classList.toggle('saving',kind==='saving');dot.classList.toggle('error',kind==='error')}if(label)label.textContent=text||'Live-sync готовий'}
function renderOwnerRoom(){
  if(!roomCfg)return;const stage=$('ownerRoomStage');if(!stage)return;const level=String(ownerRoomState.level),cfg=roomCfg.levels[level]||roomCfg.levels[0],layout=ownerRoomState.layouts[level]||roomCfg.defaults[level];
...
function moveOwnerRoomDrag(event){const drag=ownerRoomState.drag;if(!drag)return;const box=ownerRoomState.layouts[String(ownerRoomState.level)][drag.slot];box.x=roomClamp(drag.x+(event.clientX-drag.startX)/drag.rect.width*100,0,92);box.y=roomClamp(drag.y+(event.clientY-drag.startY)/drag.rect.height*100,0,90);roomApplyBox(drag.node,box,drag.slot);requestOwnerRoomLivePublish()}
function endOwnerRoomDrag(){const drag=ownerRoomState.drag;if(!drag)return;drag.node.onpointermove=null;drag.node.onpointerup=null;drag.node.onpointercancel=null;ownerRoomState.drag=null;publishOwnerRoom(true)}
function scheduleOwnerRoomPublish(commit=false){clearTimeout(ownerRoomState.saveTimer);ownerRoomState.saveTimer=setTimeout(()=>publishOwnerRoom(commit),180)}
function requestOwnerRoomLivePublish(){const now=Date.now(),wait=Math.max(0,260-(now-ownerRoomState.lastLiveAt));if(wait<=0){ownerRoomState.lastLiveAt=now;publishOwnerRoom(false);return}clearTimeout(ownerRoomState.saveTimer);ownerRoomState.saveTimer=setTimeout(()=>{ownerRoomState.lastLiveAt=Date.now();publishOwnerRoom(false)},wait)}
async function publishOwnerRoom(commit=false){
  if(!roomCfg)return false;clearTimeout(ownerRoomState.saveTimer);if(ownerRoomState.publishBusy){ownerRoomState.publishQueued=true;ownerRoomState.publishQueuedCommit=ownerRoomState.publishQueuedCommit||Boolean(commit);return true}const payload=JSON.stringify({layouts:ownerRoomState.layouts,commit:Boolean(commit)});if(!commit&&payload===ownerRoomState.lastSent)return true;ownerRoomState.publishBusy=true;roomSetLiveStatus('saving','Публікую зміни…');try{const d=await api('/api/owner/room-layout',{method:'PUT',body:payload});ownerRoomState.revision=Number(d.revision||ownerRoomState.revision);ownerRoomState.updatedAt=d.updatedAt||ownerRoomState.updatedAt;ownerRoomState.configured=true;ownerRoomState.lastSent=payload;$('roomRevision').textContent='r'+ownerRoomState.revision;$('roomUpdated').textContent=new Date(ownerRoomState.updatedAt).toLocaleString('uk-UA');roomSetLiveStatus('ok','Live-sync активний · r'+ownerRoomState.revision);if(commit)$('ownerRoomResult').textContent='Опубліковано для всіх користувачів: r'+ownerRoomState.revision;return true}catch(error){roomSetLiveStatus('error','Помилка live-sync');$('ownerRoomResult').textContent=error.message;return false}finally{ownerRoomState.publishBusy=false;if(ownerRoomState.publishQueued){const queuedCommit=ownerRoomState.publishQueuedCommit;ownerRoomState.publishQueued=false;ownerRoomState.publishQueuedCommit=false;setTimeout(()=>publishOwnerRoom(queuedCommit),0)}}}
async function loadOwnerRoomLayout(){if(!roomCfg)return;try{const d=await api('/api/owner/room-layout');ownerRoomState.layouts=roomNormalize(d.configured?d.layouts:roomCfg.defaults);ownerRoomState.revision=Number(d.revision||0);ownerRoomState.updatedAt=d.updatedAt||'';ownerRoomState.configured=!!d.configured;ownerRoomState.lastSent='';renderOwnerRoom();roomSetLiveStatus('ok',d.configured?'Live-sync активний · r'+ownerRoomState.revision:'Готово до першої публікації')}catch(error){roomSetLiveStatus('error','Не вдалося завантажити layout')}}
function nudgeOwnerRoom(dx,dy){const box=roomBox();if(!box)return;box.x=roomClamp(box.x+dx,0,92);box.y=roomClamp(box.y+dy,0,90);renderOwnerRoom();scheduleOwnerRoomPublish(false)}
function resetOwnerRoomObject(){const level=String(ownerRoomState.level),slot=ownerRoomState.selected;ownerRoomState.layouts[level][slot]=roomClone(roomCfg.defaults[level][slot]);renderOwnerRoom();publishOwnerRoom(true)}
function resetOwnerRoomLevel(){const level=String(ownerRoomState.level);ownerRoomState.layouts[level]=roomClone(roomCfg.defaults[level]);renderOwnerRoom();publishOwnerRoom(true)}
function exportOwnerRoom(){const blob=new Blob([JSON.stringify({version:'12.22.8',exportedAt:new Date().toISOString(),layouts:ownerRoomState.layouts},null,2)],{type:'application/json'}),a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='myhabbit-owner-room-layout-'+new Date().toISOString().slice(0,10)+'.json';a.click();setTimeout(()=>URL.revokeObjectURL(a.href),500)}
async function importOwnerRoomFile(file){try{const parsed=JSON.parse(await file.text());ownerRoomState.layouts=roomNormalize(parsed.layouts||parsed.roomLayoutMaster||parsed.roomMaster?.layouts||parsed);renderOwnerRoom();await publishOwnerRoom(true);$('ownerRoomResult').textContent='JSON імпортовано та одразу опубліковано'}catch(error){$('ownerRoomResult').textContent='Не вдалося імпортувати JSON: '+error.message}}
function ownerRoomPresetKey(n){return 'myhabbit:owner-console:room-preset:'+n}
function renderOwnerRoomPresets(){$('ownerRoomPresets').innerHTML=[1,2,3].map(n=>'<button class="ghost" data-owner-room-preset-save="'+n+'">Зберегти варіант '+n+'</button><button class="ghost" data-owner-room-preset-load="'+n+'">Підвантажити '+n+'</button>').join('');$('ownerRoomPresets').querySelectorAll('[data-owner-room-preset-save]').forEach(b=>b.onclick=()=>{localStorage.setItem(ownerRoomPresetKey(b.dataset.ownerRoomPresetSave),JSON.stringify(ownerRoomState.layouts));$('ownerRoomResult').textContent='Варіант '+b.dataset.ownerRoomPresetSave+' збережено в owner-console'});$('ownerRoomPresets').querySelectorAll('[data-owner-room-preset-load]').forEach(b=>b.onclick=()=>{const raw=localStorage.getItem(ownerRoomPresetKey(b.dataset.ownerRoomPresetLoad));if(!raw){$('ownerRoomResult').textContent='Варіант '+b.dataset.ownerRoomPresetLoad+' порожній';return}ownerRoomState.layouts=roomNormalize(JSON.parse(raw));renderOwnerRoom();publishOwnerRoom(true)})}
function bindOwnerRoomControls(){if(!roomCfg||!$('ownerRoomStage'))return;renderOwnerRoomPresets();$('ownerRoomSize').oninput=e=>{const box=roomBox();if(!box)return;const next=roomClamp(e.target.value,5,80);if(ownerRoomState.selected!=='teddy'&&Number.isFinite(Number(box.h)))box.h=roomClamp(box.h*next/box.w,3,95);box.w=next;$('ownerRoomSizeValue').textContent=(Math.round(next*10)/10)+'%';const node=$('ownerRoomStage').querySelector('[data-owner-room-slot="'+ownerRoomState.selected+'"]');roomApplyBox(node,box,ownerRoomState.selected);scheduleOwnerRoomPublish(false)};$('ownerRoomSize').onchange=()=>publishOwnerRoom(true);$('ownerRoomZ').oninput=e=>{const box=roomBox();if(!box)return;box.z=Math.trunc(roomClamp(e.target.value,1,40));$('ownerRoomZValue').textContent=String(box.z);const node=$('ownerRoomStage').querySelector('[data-owner-room-slot="'+ownerRoomState.selected+'"]');roomApplyBox(node,box,ownerRoomState.selected);scheduleOwnerRoomPublish(false)};$('ownerRoomZ').onchange=()=>publishOwnerRoom(true);$('ownerRoomUp').onclick=()=>nudgeOwnerRoom(0,-.5);$('ownerRoomDown').onclick=()=>nudgeOwnerRoom(0,.5);$('ownerRoomLeft').onclick=()=>nudgeOwnerRoom(-.5,0);$('ownerRoomRight').onclick=()=>nudgeOwnerRoom(.5,0);$('ownerRoomResetObject').onclick=resetOwnerRoomObject;$('ownerRoomResetLevel').onclick=resetOwnerRoomLevel;$('ownerRoomPublish').onclick=()=>publishOwnerRoom(true);$('ownerRoomReload').onclick=loadOwnerRoomLayout;$('ownerRoomExport').onclick=exportOwnerRoom;$('ownerRoomImport').onclick=()=>$('ownerRoomImportFile').click();$('ownerRoomImportFile').onchange=e=>{const file=e.target.files?.[0];if(file)importOwnerRoomFile(file);e.target.value=''}}
...
function initOwnerFolds(){
  const configs=[
    ...
    ['room-layout-section','Головний вигляд кімнати','Live master-layout для ПК і телефона'],
