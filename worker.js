...
function roomPersistDraft(){if(!roomCfg)return;try{localStorage.setItem(ownerRoomDraftKey,JSON.stringify({version:'12.23.2',savedAt:new Date().toISOString(),baseRevision:Number(ownerRoomState.revision||0),layouts:ownerRoomState.layouts}))}catch{}}
...
function renderOwnerRoom(){
  ...
  if(!roomCfg){stage.innerHTML='<div style="display:grid;place-items:center;height:100%;padding:24px;text-align:center;color:#934d4d;font-weight:800">Не завантажено конфігурацію кімнати. Оновіть сторінку після встановлення 12.23.2.</div>';roomSetLiveStatus('error','Конфігурацію кімнати не завантажено');return}
